...
export class WalletService {
  ...
  async depositWallet(
    ...
    return this.transactionService.findTransactionById(newTransaction.id);
  }
  ...
  async findAndUpdateWallet(walletAddress: string, update: Record<any, any> = {}) {
    const updated = await this.walletRepository
      .createQueryBuilder()
      .useTransaction(true)
    ...
    if (!updated.affected) throw new BadRequestException('Cant not update Wallet');
  }
