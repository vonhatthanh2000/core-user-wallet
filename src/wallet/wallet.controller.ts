...
export class WalletController implements WalletServiceControllerGrpc {
  ...
  async depositWallet(request: DepositInput): Promise<DepositResponse> {
    // check if currency of request is same as currency of wallet or not
    const userWallet = await this.walletService.findWallet(request.walletAddress);
    if (userWallet.currency != request.currency) throw new RpcException('This wallet is not support this currency');
    ...
    return DepositResponse.fromJSON(deposit);
  }
